...
public class HairAdapter extends BaseRVAdapter<HairAdapter.ViewHolder> {
    ...
    /**
     */
    public HairAdapter(Context context, RequestManager requestManager) {
     ...
     *
     */
    public void addAll(List<ItemBean> tmp, int checked) {
        list=tmp;
        lastCheck = checked;
    ...
    @Override
    public HairAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    private void updateCheckProgress(ViewHolder holder, int position) {
        ...
        if (position == lastCheck && (TextUtils.isEmpty(info.getFile()) || !TextUtils.isEmpty(info.getLocalPath()))) { //选中
            holder.progressLayout.setVisibility(View.GONE);
            holder.mImageView2.setChecked(true);
        } else if (null != mProgressCallBack && mProgressCallBack.getMap() != null && mProgressCallBack.getMap().containsKey(info.getFile())) { //下载中
            int pro = mProgressCallBack.getMap().get(info.getFile());
        ...
        GlideUtils.setCover(mRequestManager, holder.mImageView2, info.getCover());
    }

    @Override
    public void onBindViewHolder(HairAdapter.ViewHolder holder, int position) {
