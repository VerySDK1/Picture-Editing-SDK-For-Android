 ...
 * 需要超出父控件
 */
public class ExtSeekBar extends androidx.appcompat.widget.AppCompatSeekBar {
    ...
    private void init(Context context) {
        mDrawable = ContextCompat.getDrawable(context, R.drawable.pecom_ic_bar_pop);
        mPoint = ContextCompat.getDrawable(context, R.drawable.pecom_ic_bar_thumb);
        ...
        //颜色
        mProgressColor = ContextCompat.getColor(context, R.color.pesdk_main_color);
        mTextColor = ContextCompat.getColor(context, R.color.pesdk_main_color);
        ...
        mProPaint.setColor(mProgressColor);
        //背景
        mBgPaint = new Paint();
    ...
    public void setThumb(int drawable) {
        mPoint = ContextCompat.getDrawable(getContext(), drawable);
        mPointW = mPoint.getIntrinsicWidth();
        mPointH = mPoint.getIntrinsicHeight();
    }

    private RectF mUnableRectF = new RectF();
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        //背景
        mBgRect.set(mProMargin / 2.0f, mPointH / 2.0f - 2.5f, getWidth() - mProMargin / 2.0f, mPointH / 2.0f + 2.5f);
        ...
        if (!isEnabled()) {
            ...
            mUnableRectF.set(Math.min(mBgRect.left, mProgressRectF.left), Math.min(mBgRect.top, mProgressRectF.top),
                    Math.max(mBgRect.right, mProgressRectF.right), Math.max(mBgRect.bottom, mProgressRectF.bottom));
            canvas.drawRoundRect(mUnableRectF, 5, 5, mBgPaint);
        }
     ...
     * 进度颜色
     */
    public void setProgressColor(int proColor) {
